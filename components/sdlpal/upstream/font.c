...
#define PAL_UNICODE_COUNT 65536

static unsigned char (*unicode_font)[32];
static unsigned char *font_width;
...
static void PAL_LoadEmbeddedFont(void)
{
	...
	for (i = 0; i < nChars; i++)
	{
		...
		if (w >= PAL_UNICODE_COUNT) { continue; }
...
#define PAL_LOAD_INTERNAL_FONT(fontdata, height) \
   { \
    ...
    unicode_font = heap_caps_calloc(PAL_UNICODE_COUNT, sizeof(unicode_font[0]),
                                    MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    font_width = heap_caps_malloc(PAL_UNICODE_COUNT,
                                  MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (unicode_font == NULL || font_width == NULL)
    {
       heap_caps_free(unicode_font);
       heap_caps_free(font_width);
       unicode_font = NULL;
       font_width = NULL;
       return -1;
    }

    memset(font_width, 16, PAL_UNICODE_COUNT);
    PAL_LoadISOFont();

    size_t fonts = PAL_UNICODE_COUNT;
