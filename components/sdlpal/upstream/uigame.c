...
static WORD GetSavedTimes(int iSaveSlot)
{
	...
	if (fp == NULL)
	{
		/* PAL_SaveGame_Common keeps the previous save under this name during
		 * its atomic replacement window. Make that recovery copy visible in
		 * the load menu after a reset at exactly that point. */
		fp = UTIL_OpenFileAtPath(gConfig.pszSavePath, PAL_va(0, "%d.rpg.bak", iSaveSlot));
	}
	WORD wSavedTimes = 0;
	if (fp != NULL)
	{
