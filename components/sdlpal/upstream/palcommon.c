...
--*/
{
   ...
   int read_len = PAL_MKFReadChunk(buf, len, uiChunkNum, fp);
   if (read_len != len)
   {
      /* Never feed a short or failed SD-card read to YJ1/YJ2. The
       * decompressors trust their input and malformed data can otherwise
       * overwrite the heap before the caller sees an error. */
      free(buf);
      return -4;
   }
