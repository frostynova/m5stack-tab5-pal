...
static SDL_Texture *VIDEO_CreateTexture(int width, int height)
{
    ...
    gTextureRect.w = 320;
    gTextureRect.h = 200;
    VIDEO_SetupTouchArea(1280, 720, 1152, 720);
    return SDL_CreateTexture(gpRenderer, SDL_PIXELFORMAT_ARGB8888,
...
--*/
{
   ...
   {
#if PAL_TAB5
      printf("Tab5 SDL_CreateWindow failed: %s\n", SDL_GetError());
#endif
      return -1;
   }
...
# endif
   ...
   {
#if PAL_TAB5
      printf("Tab5 SDL_CreateRenderer failed: %s\n", SDL_GetError());
#endif
      return -1;
   }
...
{
#if PAL_TAB5
    const void *frame = PAL_Tab5TouchComposeOverlay(gpScreenReal->pixels,
                                                    gpScreenReal->pitch);
    const esp_err_t result = esp_bsp_sdl_present_pal_frame(frame);
    if (result != ESP_OK)
    {
        printf("Tab5 PPA frame failed: %s\n", esp_err_to_name(result));
    }
    return;
#else
	void *texture_pixels;
