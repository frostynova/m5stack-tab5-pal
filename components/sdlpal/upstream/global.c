...
#ifdef ESP_PLATFORM
#include "esp_log.h"
#define PAL_SAVE_LOGI(...) ESP_LOGI("sdlpal_save", __VA_ARGS__)
#define PAL_SAVE_LOGE(...) ESP_LOGE("sdlpal_save", __VA_ARGS__)
#else
#define PAL_SAVE_LOGI(...) do { fprintf(stderr, "SDLPal save: "); fprintf(stderr, __VA_ARGS__); fprintf(stderr, "\n"); } while (0)
#define PAL_SAVE_LOGE(...) do { fprintf(stderr, "SDLPal save error: "); fprintf(stderr, __VA_ARGS__); fprintf(stderr, "\n"); } while (0)
#endif
...
)
{
	...
	size_t minimum_size = size - sizeof(EVENTOBJECT) * MAX_EVENT_OBJECTS;
	size_t n = 0;
	...
	/* A reset can happen after the old save was moved aside but before the
	 * freshly synced temporary file was installed. In that narrow window the
	 * backup is still the last known-good save. It is also useful if a final
	 * save file is unexpectedly truncated. */
	if (n < minimum_size)
	{
		snprintf(backup_name, sizeof(backup_name), "%d.rpg.bak", iSaveSlot);
		fp = UTIL_OpenFileAtPath(gConfig.pszSavePath, backup_name);
		if (fp != NULL)
		{
			n = fread(s, 1, size, fp);
			fclose(fp);
	...
	if (n < minimum_size)
	{
		PAL_SAVE_LOGE("load slot %d failed: got %zu bytes, need at least %zu", iSaveSlot, n, minimum_size);
		return FALSE;
	}

	PAL_SAVE_LOGI("loaded slot %d from %s (%zu bytes)", iSaveSlot,
		using_backup ? "backup" : "primary", n);
...
)
{
	...
	char backup_name[32];
	char final_path[PAL_GLOBAL_BUFFER_SIZE];
	char temporary_path[PAL_GLOBAL_BUFFER_SIZE];
	char backup_path[PAL_GLOBAL_BUFFER_SIZE];
	bool old_save_moved = false;
	size_t i;
	...
	snprintf(final_name, sizeof(final_name), "%d.rpg", iSaveSlot);
	snprintf(temporary_name, sizeof(temporary_name), "%d.rpg.tmp", iSaveSlot);
	snprintf(backup_name, sizeof(backup_name), "%d.rpg.bak", iSaveSlot);
	UTIL_CombinePath(final_path, sizeof(final_path), 2, gConfig.pszSavePath, final_name);
	UTIL_CombinePath(temporary_path, sizeof(temporary_path), 2, gConfig.pszSavePath, temporary_name);
	UTIL_CombinePath(backup_path, sizeof(backup_path), 2, gConfig.pszSavePath, backup_name);

	/* Keep the existing save untouched until the replacement is complete on
	 * the card. This avoids turning a short write into a corrupt save slot. */
	unlink(temporary_path);
	fp = fopen(temporary_path, "wb");
	if (fp == NULL)
	{
		PAL_SAVE_LOGE("slot %d temporary open failed: %s", iSaveSlot, strerror(errno));
		return;
	}

	if (fwrite(s, 1, i, fp) != i)
	{
		PAL_SAVE_LOGE("slot %d short write: %s", iSaveSlot, strerror(errno));
		goto SaveFailed;
	}
	if (fflush(fp) != 0 || fsync(fileno(fp)) != 0)
	{
		PAL_SAVE_LOGE("slot %d flush/sync failed: %s", iSaveSlot, strerror(errno));
		goto SaveFailed;
	}
	if (fclose(fp) != 0)
	{
		fp = NULL;
		PAL_SAVE_LOGE("slot %d temporary close failed: %s", iSaveSlot, strerror(errno));
		goto SaveFailed;
	}
	fp = NULL;

	/* FatFS does not replace an existing destination during rename. Move the
	 * previous primary to a backup first. If only a recovery backup exists,
	 * leave it alone until the new primary has been installed successfully. */
	struct stat final_info;
	if (stat(final_path, &final_info) == 0)
	{
		if (unlink(backup_path) != 0 && errno != ENOENT)
		{
			PAL_SAVE_LOGE("slot %d could not clear stale backup: %s", iSaveSlot, strerror(errno));
			goto SaveFailed;
		}
		if (rename(final_path, backup_path) != 0)
		{
			PAL_SAVE_LOGE("slot %d could not preserve old save: %s", iSaveSlot, strerror(errno));
			goto SaveFailed;
		}
		old_save_moved = true;
	}
	...
	{
		PAL_SAVE_LOGE("slot %d could not inspect old save: %s", iSaveSlot, strerror(errno));
		goto SaveFailed;
	}

	if (rename(temporary_path, final_path) != 0)
	{
		PAL_SAVE_LOGE("slot %d could not install new save: %s", iSaveSlot, strerror(errno));
		if (old_save_moved && rename(backup_path, final_path) != 0)
		{
			PAL_SAVE_LOGE("slot %d could not restore backup: %s", iSaveSlot, strerror(errno));
		}
		goto SaveFailed;
	}
	...
	if (unlink(backup_path) != 0 && errno != ENOENT)
	{
		PAL_SAVE_LOGE("slot %d saved, but stale backup cleanup failed: %s", iSaveSlot, strerror(errno));
	}
	PAL_SAVE_LOGI("saved slot %d atomically (%zu bytes)", iSaveSlot, i);
	return;
