...
- ES8388 以 22.05 kHz 单声道输出音频
- 通过 DOSBox 整数 OPL 核心播放 Windows 版音效和 RIX 音乐
- 在左侧黑边显示电池电压、估算电量和带方向的电流
- 在右侧黑边提供触摸音量控制
- 通过 Tab5 电源扩展器启用 IP2326 充电和 QC 协商
...
| Y | SDLPal 支持的强制/防御操作 |
| AUTO | 开启或关闭自动战斗 |
| 左侧黑边 | 显示估算电量；点击后查看电压和带正负号的电流 |
| 右侧黑边 `+` / `-` | 将音乐和音效音量提高或降低 10% |

方向键使用了扩大的触摸判定区域。手指保持按下时，可以直接滑向另一个方向。

电池数据来自板载 INA226，每秒刷新一次。正电流表示正在充电，负电流表示正在
放电。百分比是根据电压曲线得到的估算值，并非电量计的直接读数，因此在充电或
负载变化时可能波动。
...
- 移植层按照官方充电顺序初始化第二个 Tab5 电源扩展器：先启用 QC 协商，再拉高
  `CHG_EN`；读改写过程中保留所有无关的电源输出位。
- 板载 INA226 读取使用官方 5 毫欧分流电阻参数；两侧状态控件只占用黑边，
  不覆盖 320x200 游戏画面。
